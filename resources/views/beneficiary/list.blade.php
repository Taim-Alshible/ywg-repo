<x-layout>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            <form method="GET" action="{{ route('beneficiary.search') }}" class="flex flex-col gap-3 sm:flex-row sm:items-center">
                <label for="search" class="sr-only">بحث عن مستفيد</label>
                <div class="flex flex-1 flex-col gap-2 sm:flex-row sm:items-center">
                    <div class="flex flex-1 items-stretch rounded-md border border-teal-500 shadow-sm overflow-hidden">
                        <span class="flex items-center px-4 py-2 bg-teal-500 text-white font-semibold dark:bg-teal-600">بحث عن مستفيد</span>
                        <input id="search" name="search" type="text" value="{{ old('search', $searchTerm ?? '') }}"
                            placeholder="أدخل اسم المستفيد أو رقم الهاتف"
            ...
            <div class="flex justify-end">
                <a href="{{ route('beneficiary.pdf', array_filter(['search' => $searchTerm])) }}"

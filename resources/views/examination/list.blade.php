<x-layout>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            <form method="GET" action="{{ route('examination.search') }}"
                class="flex flex-col gap-3 sm:flex-row sm:items-center">
                <label for="search" class="sr-only">بحث عن فحص</label>
                <div class="flex flex-1 flex-col gap-2 sm:flex-row sm:items-center">
                    <div class="flex flex-1 items-stretch rounded-md border border-teal-500 shadow-sm overflow-hidden">
                        <span class="flex items-center px-4 py-2 bg-teal-500 text-white font-semibold dark:bg-teal-600">بحث عن فحص</span>
                        <input id="search" name="search" type="text" value="{{ old('search', $searchTerm ?? '') }}"
                            placeholder="أدخل اسم المريض أو تفاصيل الفحص" class="flex-1 px-3 py-2 border-0 bg-white text-gray-900 focus:ring-0 focus:outline-none dark:bg-gray-700 dark:text-gray-100" />
                    </div>
            ...
            <div class="flex justify-end">
                <a href="{{ route('examination.pdf', array_filter(['search' => $searchTerm])) }}"
                ...
                <table class="min-w-full">
                    ...
                    <tbody class="bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200">
                        ...
                        @empty
                            <tr>
                                <td colspan="3"
                                    class="px-4 py-3 text-center border-t border-gray-200 text-gray-600 dark:text-gray-200 dark:border-gray-700">
                                    لا توجد فحوصات مطابقة لنتائج البحث.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
